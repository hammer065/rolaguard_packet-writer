...
class Packet(Base):
    ...
    def rows_quantity(cls):
        return session.query(func.count(Packet.id)).scalar()
...
class DeviceAuthData(Base):
    ...
    data_collector_id = Column(BigIntegerType, ForeignKey("data_collector.id"), nullable=False)
    organization_id = Column(BigIntegerType, ForeignKey("organization.id"), nullable=False)
    app_key_id = Column(BigIntegerType, ForeignKey("app_key.id"), nullable=False)
    device_id = Column(BigIntegerType, ForeignKey("device.id"), nullable=True)
    device_session_id = Column(BigIntegerType, ForeignKey("device_session.id"), nullable=True)
    created_at = Column(DateTime(timezone=True), nullable=False)
    ...
    @classmethod
    def find_one_by_device_id_and_data_collector_id_and_app_key_id(cls, device_id, data_collector_id, app_key_id):
        return session.query(cls).filter(cls.device_id == device_id, cls.data_collector_id == data_collector_id).filter(cls.app_key_id == app_key_id).first()

    @classmethod
    def find_one_by_dev_addr_and_organization_id(cls, dev_addr, organization_id):
        return session.query(cls).filter(cls.device_session_id == DeviceSession.id).filter(DeviceSession.dev_addr == dev_addr).filter(cls.organization_id == organization_id).all()

    @classmethod
    def find_one_pending_request_on_last_seconds_by_data_collector_id_and_app_key_id(cls, data_collector_id, app_key_id, last_seconds_ago_date):
        return session.query(cls).filter(cls.data_collector_id == data_collector_id).filter(cls.app_key_id == app_key_id).filter(cls.join_accept==None).filter(cls.created_at > last_seconds_ago_date).first()
...
class AppKey(Base):
    ...
    @classmethod
    def find_one_by_key_and_organization_id(cls, key, organization_id):
        return session.query(cls).filter(cls.key == key, cls.organization_id == organization_id).first()

    @classmethod        
    def find_key_only_by_organization_id(cls, organization_id):
        keys = session.query(cls).filter(cls.organization_id == organization_id).all()
        return list(map(lambda app_key: app_key.key, keys))
